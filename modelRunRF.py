...
parser.add_argument("-f", "--fraction", dest="fraction", help="Fraction of dataset to use. Ex: for 1/4 use -f 4", default=1, type=int)
...
parser.add_argument("-j", "--jobs", dest="n_jobs", help="Number of parallel jobs to use", default=-1, type=int)
...
model_dict = {
            ...
            "RandomForest": RandomForestClassifier(random_state=args.seed, n_estimators=1000, n_jobs=args.n_jobs),
...
X_resampled, y_resampled = resample(X_train, y_train, n_samples=rank_sample_size, random_state=args.seed, stratify=y_train)
...
results_path = f"results/grid_search_results/RF.csv"
results.round(3).to_csv(results_path, mode='a', index=False, header=not os.path.exists(results_path))
